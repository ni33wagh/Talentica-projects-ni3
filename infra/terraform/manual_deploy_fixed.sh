#!/bin/bash
# Manual CI/CD Health Dashboard Deployment Script (Fixed)
...
# Create backend Python file
echo "📝 Creating backend application..."
sudo mkdir -p backend
sudo tee backend/main.py > /dev/null << 'EOF'
...
sudo tee backend/requirements.txt > /dev/null << 'EOF'
fastapi==0.104.1
uvicorn==0.24.0
requests==2.31.0
...
sudo mkdir -p frontend/views frontend/public
...
sudo tee frontend/package.json > /dev/null << 'EOF'
{
...
# Create frontend EJS template
sudo tee frontend/views/dashboard.ejs > /dev/null << 'EOF'
<!DOCTYPE html>
<html>
<head>
    <title><%= title %></title>
...
# Create docker-compose.yml with fixed syntax
